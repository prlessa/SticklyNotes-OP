...
const cache = {
  ...
  // Função para cachear painel
  cachePanel: async (panelId, panelData) => {
    if (!redisClient) return null;
    try {
      const key = `panel:${panelId}`;
      const value = JSON.stringify(panelData);
      return await redisClient.setex(key, config.cache?.panelTtl || 300, value);
    } catch (error) {
      console.warn('Cache panel error:', error.message);
      return null;
    }
  },

  // Função para obter painel do cache
  getCachedPanel: async (panelId) => {
    if (!redisClient) return null;
    try {
      const key = `panel:${panelId}`;
      ...
      console.warn('Get cached panel error:', error.message);
      return null;
    }
  }
};
